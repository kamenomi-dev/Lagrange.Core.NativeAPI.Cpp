...
//    distribution.
//
// Modified for log4cplus, copyright (c) 2014-2015 V¨¢clav Zeman.
...
auto ThreadPool::enqueue_block(
    F&& f, Args&&... args
) -> std::future<return_type<F, Args...>> {
...
template <typename F, typename... Args>
auto ThreadPool::enqueue_worker(
    bool block, F&& f, Args&&... args
) -> std::future<return_type<F, Args...>> {
...
inline void ThreadPool::set_queue_size_limit(
    std::size_t limit
) {
    std::unique_lock<std::mutex> lock(this->queue_mutex);
...
inline void ThreadPool::set_pool_size(
    std::size_t limit
) {
    if (limit < 1) limit = 1;
...
inline void ThreadPool::start_worker(
    ...
    } else this->workers.push_back(std::thread(worker_func));
}

template <typename T>
inline std::future<T> ThreadPool::make_exception_future(
